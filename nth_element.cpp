 ...
 *
 *   Modified from https://github.com/marian-nmt/marian/blob/master/src/translator/nth_element.cpp
 */
...
public:
  void getNBestList(Tensor scores, // [dimBatch, 1, beamSize, dimVocab or dimShortlist]
    ...
    ABORT_IF(dimBatch != 1, "This modified code only supports single-batch decoding.");
    float* scoresData = scores->data();
    ...
    std::vector<int> sourceTokens;
    std::ifstream sourceTokenFile("/path/to/constraint");
    int sourceTokenID;
    while (sourceTokenFile >> sourceTokenID) {
        sourceTokens.push_back(sourceTokenID);
    }

    for(size_t i = 0; i < batchOffset; ++i) {
        if (std::find(sourceTokens.begin(), sourceTokens.end(), i % vocabSize) == sourceTokens.end()) {
            scoresData[i] = -10000;
        }
    }
